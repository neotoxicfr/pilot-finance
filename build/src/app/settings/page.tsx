...
export default function SettingsPage() {
  ...
  const handleStartMfaSetup = async () => { 
      setMfaLoading(true); 
      const data = await generateMfaSecretAction(); 
      if (data) {
  ...
  const handleConfirmMfa = async () => { 
      ...
      setMfaLoading(true); 
      setMfaError(''); 
      const res = await enableMfaAction(mfaSetupData.secret, mfaCode); 
      if(res?.error) {
                        ...
                        {!mfaSetupData ? (
                            ...
                            <div className="space-y-5 animate-in fade-in slide-in-from-top-4">
                                <div className="text-center bg-white p-4 rounded-xl w-fit mx-auto border border-border">
                                    <Image src={mfaSetupData.imageUrl} alt="QR Code A2F" width={192} height={192} className="w-48 h-48" />
                                </div>
